...
class CaseCreation(object):
    def setUp(self):
        self.address = Address.objects.create(
            street="street name", city="city name", postcode="1234", country="US"
        )
        ...
        self.content_type = ContentType.objects.create(app_label="cases1", model="case")
...
class CaseRemoveTestCase(CaseCreation, TestCase):
    ...
    def test_case_delete(self):
        response = self.client.post(
            "/cases/" + str(self.case.id) + "/remove/", {"case_id": self.case.id}
        )
        self.assertEqual(response.status_code, 200)
...
class CaseUpdateTestCase(CaseCreation, TestCase):
    ...
    def test_case_update_html(self):
        response = self.client.post(
            "/cases/" + str(self.case.id) + "/edit_case/", {"hiddenval": self.case.id}
        )
        # self.assertTemplateUsed(response, 'create_cases.html')
        self.assertEqual(response.status_code, 200)
...
class TestCasesListViewForUser(CaseCreation, TestCase):
    def test_queryset_for_user(self):
        ...
        self.comment_1 = Comment.objects.create(
            comment="test comment", case=self.case, commented_by=self.usermp
        )
    ...
    def test_case_update_test(self):
        ...
        response = self.client.post(
            reverse("cases:edit_case", args=(self.case.id,)),
            {
                "name": "some case",
                "status": "New",
                "priority": "Low",
    ...
    def test_permissions(self):
        ...
        response = self.client.get(
            reverse("cases:edit_case", args=(self.case_new.id,)),
            {
                "name": "some case",
                "status": "New",
                "priority": "Low",
        ...
        response = self.client.post(
            reverse("cases:remove_case", args=(self.case.id,)),
            {"case_id": self.case_new.id},
        )
        self.assertEqual(response.status_code, 403)

        response = self.client.post(
            reverse("cases:close_case"), {"case_id": self.case.id}
        )
        self.assertEqual(response.status_code, 403)
        ...
        response = self.client.post(
            reverse("cases:add_case") + "?view_account={}".format(self.account.id),
            {
                "name": "team case",
                "status": "New",
                "priority": "Low",
        ...
        response = self.client.get(
            reverse("cases:view_case", args=(self.case_without_account.id,))
        )
        self.assertEqual(200, response.status_code)
        ...
        self.client.login(username="joedoeCase7@user.com", password="password")
        response = self.client.get(
            reverse("cases:view_case", args=(self.case_user.id,))
        )
        self.assertEqual(200, response.status_code)

        self.case_user_1 = Case.objects.create(
            name="case user authorization",
            case_type="Problem",
            status="New",
            priority="Low",
            description="something",
        ...
        response = self.client.get(
            reverse("cases:view_case", args=(self.case_user_1.id,))
        )
        ...
        self.client.login(username="joedoeCase7@user.com", password="password")
        response = self.client.get(
            reverse("cases:edit_case", args=(self.case_user.id,))
        )
        self.assertEqual(200, response.status_code)
        ...
        response = self.client.get(
            reverse("cases:remove_case", args=(self.case_user_1.id,))
            + "?view_account={}".format(self.account.id)
        )
        self.assertEqual(403, response.status_code)
        ...
        self.client.login(email="johnDoeCase@example.com", password="password")
        response = self.client.get(
            reverse("cases:remove_case", args=(self.case_user.id,))
            + "?view_account={}".format(self.account.id)
        )
        ...
        response = self.client.post(
            reverse("cases:close_case") + "?view_account={}".format(self.account.id),
            {"case_id": self.case.id},
        )
        self.assertEqual(403, response.status_code)
        data = {"caseid": self.case.id, "comment": ""}
        ...
        self.client.login(email="johnDoeCase@example.com", password="password")
        response = self.client.get(
            reverse("cases:select_contacts") + "?account={}".format(self.account.id)
        )
        self.assertEqual(200, response.status_code)
        ...
        data = {"comment": "", "caseid": self.case.id, "commentid": self.comment.id}

...
class AccountCreateTest(object):
    def setUp(self):
        ...
        self.user = User.objects.create(
            first_name="johnAccount",
            username="johnDoeAccount",
            email="johnAccount@example.com",
            role="ADMIN",
...
class CommentTestCase(AccountCreateTest, TestCase):
    ...
    def test_comment_create(self):
        response = self.client.post(
            "/accounts/comment/add/",
            {"accountid": self.account.id, "comment": self.comment.id},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_comment_creation(self):
        ...
        response = self.client.post(
            "/accounts/comment/add/",
            {"accountid": self.account.id, "comment": "comment"},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_comment_edit(self):
        self.client.login(email="mp@micropyramid.com", password="mp")
        response = self.client.post(
            "/accounts/comment/edit/",
            {"commentid": self.comment.id, "comment": "comment"},
        )
        self.assertEqual(response.status_code, 200)
...
class AttachmentTestCase(AccountCreateTest, TestCase):
    ...
    def test_attachment_valid(self):
        ...
        response = self.client.post(
            "/accounts/attachment/add/",
            {
                "accountid": self.account.id,
                "attachment": SimpleUploadedFile(upload_file.name, upload_file.read()),
            },
        )
        self.assertEqual(response.status_code, 200)
...
class TagCreateTest(object):
    def setUp(self):
        self.tag = Tags.objects.create(name="tag", slug="tag1")
...
class TestCreateLeadPostView(AccountCreateTest, TestCase):
    ...
    def test_update_lead_post_status(self):
        upload_file = open("static/images/user.png", "rb")
        response = self.client.post(
            reverse("accounts:edit_account", kwargs={"pk": self.account.id}),
            {
                "name": "janedoeLead",
                "email": "janelead@example.com",
                "phone": "91123456789",
...
class test_account_forms(AccountCreateTest, TestCase):
    def test_account_form(self):
        ...
        response = self.client.get(
            reverse("accounts:create_mail") + "?account_id={}".format(self.account.id)
        )
        self.assertEqual(200, response.status_code)
...
class test_account_models(AccountCreateTest, TestCase):
    def test_account_model(self):
        ...
        self.account.save()
        self.assertEqual(
            "billing address line, street name, city name, state, 1234, United States",
            self.account.get_complete_address(),
        )
        self.account.billing_street = "billing street"
        self.account.save()
        self.assertEqual(
            "billing address line, billing street, city name, state, 1234, United States",
            self.account.get_complete_address(),
        )
        ...
        self.account.save()
        self.assertEqual("1234, United States", self.account.get_complete_address())
...
class test_account_views_list(AccountCreateTest, TestCase):
    def test_account_views(self):
        ...
        response = self.client.get(
            reverse("accounts:edit_account", args=(self.account_by_user1.id,)), {}
        )
        self.assertEqual(200, response.status_code)
        response = self.client.get(
            reverse("accounts:remove_account", args=(self.account_by_user.id,)), {}
        )
        self.assertEqual(403, response.status_code)

        response = self.client.post(
            reverse("accounts:add_comment"), {"accountid": self.account_by_user.id}
        )
        self.assertEqual(200, response.status_code)

        response = self.client.post(
            reverse("accounts:edit_comment"), {"commentid": self.comment.id}
        )
        self.assertEqual(200, response.status_code)
        ...
        response = self.client.post(
            reverse("accounts:edit_comment"),
            {"commentid": self.comment.id, "comment": ""},
        )
        self.assertEqual(200, response.status_code)
        ...
        self.client.login(email="janeAccount@example.com", password="password")
        response = self.client.post(
            reverse("accounts:remove_comment"),
            {"comment_id": self.comment.id, "comment": ""},
        )
        self.assertEqual(200, response.status_code)
        ...
        response = self.client.post(
            reverse("accounts:add_attachment"),
            {"accountid": self.account.id, "comment": ""},
        )
        self.assertEqual(200, response.status_code)

        email_str = Email.objects.create(
...
class TestAccountUserMentions(AccountCreateTest, TestCase):
    def test_account_views(self):
        ...
        self.account = Account.objects.create(
            ...
            billing_country="US",
            website="www.example.como",
            created_by=self.user_created_by,
            status="open",
            industry="SOFTWARE",
            description="Testing",
        )
        ...
        self.client.logout()
        self.client.login(
            email="janeAccountUserAssigned@example.com", password="password"
        )
        response = self.client.get(
            reverse("accounts:view_account", args=(self.account.id,))
        )
